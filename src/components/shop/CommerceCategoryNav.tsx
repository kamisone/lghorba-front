...
function CategoryDropdownItem({
  node, locale, activeCategory, expanded, onToggle, onNavigate, t,
}: {
  node: CategoryNode;
  locale: string;
    ...
    <div className={styles.nav} ref={navRef}>
      ...
      {tree.map(node => {
        ...
        return (
          <div
            key={node.id}
            ref={el => { if (el) triggerRefs.current.set(node.id, el); else triggerRefs.current.delete(node.id); }}
