...
const ProductGallery = forwardRef<ProductGalleryHandle, Props>(function ProductGallery({ media, title, compact, focusUrl }, ref) {
  ...
  // click, keyboard) is animating the main image container — see the two
  // effects below. While true, onMainImageScroll ignores every scroll event
  // outright, instead of merely racing a debounce timer against it: the
  // previous fix only cleared an already-pending timer, but the reset's own
  // smooth-scroll animation generates its own scroll events, which could
  // still schedule a *new* timer that reads an in-flight, not-yet-settled
  // scrollLeft and clobbers `current` with the wrong slide right after the
  // reset — landing, for the exact index math involved, on the slide that
  // used to be the hero (e.g. picking White after Black would settle back
  // on Black, since Black shifts to index 1 once White takes index 0).
