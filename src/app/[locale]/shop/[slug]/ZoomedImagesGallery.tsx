...
/**
 * Full-bleed, automatically scrolling filmstrip — purely decorative and
 * non-interactive (no lightbox, no click target, nothing selectable).
 * Hovering only pauses the motion; nothing is revealed. Loops seamlessly by
 * rendering every image twice back to back and animating the track by
 * exactly -50% (see .track/@keyframes in the CSS) — an identical second
 * copy means the loop restarts on a pixel-identical frame.
 */
export default function ZoomedImagesGallery({ items, title, ariaLabel, fullBleed = false }: {
   ...
   *  the caller in page.tsx) — defaults to false (fills its own container). */
  fullBleed?: boolean;
}) {
