...
export default function ManagementPage() {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) { const p = await res.json(); setPhotos(prev => [...prev, p]); }
    } finally {
              ...
              >
                ...
                <span className={styles.actionLabel}>
                  {isSending
                    ...
                    ? "Waiting…"
                    : canUnlock
                    ? releaseClicks === 0 ? "Tap 3× to unlock" : `${RELEASE_THRESHOLD - releaseClicks} tap${RELEASE_THRESHOLD - releaseClicks > 1 ? "s" : ""} left`
                    : isErr
                    ? "Failed"
                    : action.label}
                </span>
            ...
            {FINE_ACTIONS.map((action) => {
              ...
              return (
                <button
                  ...
                  disabled={!isWaiting && isBlocked}
                >
                  {isLoading ? <span className={styles.sendingSpinner} /> : <span className={styles.fineActionSign}>{action.sign}</span>}
