...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const res = await fetch(`${B}/incidents/${params.id}`, { cache: "no-store", headers: bearer(req) });
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await fetch(`${B}/incidents/${params.id}`, { method: "PATCH", cache: "no-store", headers: { "Content-Type": "application/json", ...bearer(req) }, body: JSON.stringify(body) });
    return NextResponse.json(await res.json(), { status: res.status });
  } catch { return NextResponse.json({ message: "Service unavailable" }, { status: 503 }); }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const res = await fetch(`${B}/incidents/${params.id}`, { method: "DELETE", cache: "no-store", headers: bearer(req) });
