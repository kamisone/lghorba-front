...
export async function GET(req: NextRequest, { params }: { params: { id: string; photoId: string } }) {
  try {
    const res = await fetch(`${B}/incidents/${params.id}/photos/${params.photoId}`, { cache: "no-store", headers: bearer(req), redirect: "manual" });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string; photoId: string } }) {
  try {
    const res = await fetch(`${B}/incidents/${params.id}/photos/${params.photoId}`, { method: "DELETE", cache: "no-store", headers: bearer(req) });
