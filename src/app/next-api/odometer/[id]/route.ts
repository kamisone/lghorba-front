...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const res = await fetch(`${B}/odometer/${params.id}`, { cache: "no-store", headers: bearer(req) });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const res = await fetch(`${B}/odometer/${params.id}`, { method: "DELETE", cache: "no-store", headers: bearer(req) });
