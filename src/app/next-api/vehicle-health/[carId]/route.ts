...
export async function GET(req: NextRequest, { params }: { params: { carId: string } }) {
  try {
    const res = await fetch(`${B}/vehicle-health/${params.carId}`, { cache: "no-store", headers: bearer(req) });
...
export async function PUT(req: NextRequest, { params }: { params: { carId: string } }) {
  try {
    ...
    const res = await fetch(`${B}/vehicle-health/${params.carId}`, { method: "PUT", cache: "no-store", headers: { "Content-Type": "application/json", ...bearer(req) }, body: JSON.stringify(body) });
    return NextResponse.json(await res.json(), { status: res.status });
  } catch { return NextResponse.json({ message: "Service unavailable" }, { status: 503 }); }
}
