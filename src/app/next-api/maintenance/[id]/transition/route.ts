...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await fetch(`${B}/maintenance/${params.id}/transition`, { method: "POST", cache: "no-store", headers: { "Content-Type": "application/json", ...bearer(req) }, body: JSON.stringify(body) });
