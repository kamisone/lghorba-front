...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${B}/maintenance${qs ? `?${qs}` : ""}`, { cache: "no-store", headers: bearer(req) });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${B}/maintenance`, { method: "POST", cache: "no-store", headers: { "Content-Type": "application/json", ...bearer(req) }, body: JSON.stringify(body) });
    return NextResponse.json(await res.json(), { status: res.status });
  } catch { return NextResponse.json({ message: "Service unavailable" }, { status: 503 }); }
}
